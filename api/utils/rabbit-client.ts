...
export class RabbitClient {
  ...
  async sendMessage(
    ...
    if (!this.producers[producerName]) {
      throw new ProducerNotFound(
        `There is no active producer with the name ${producerName}`
      )
    }
    try {
      await this.producers[producerName].sendToQueue(
        queue,
        Buffer.from(message)
      )
    } catch (error: any) {
  ...
  async close(): Promise<void> {
    await Promise.all(
      Object.values(this.producers).map((producer: any) => producer.close())
    )
